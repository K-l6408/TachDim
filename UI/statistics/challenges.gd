...
func _process(_delta):
	...
	if Globals.TachTotal.log10() < Globals.PCUnlocks[0]:
		$ETable.hide()
		...
		for i in 7:
			...
			Globals.format_time(Globals.PCTimes[i]) if Globals.PCTimes[i] > 0 else "N/A"
			if Globals.PCTimes.size() == i:
				Globals.PCTimes.append(-1)
			sum += Globals.PCTimes[i]
			if Globals.PCTimes[i] < 0: sum = -999
